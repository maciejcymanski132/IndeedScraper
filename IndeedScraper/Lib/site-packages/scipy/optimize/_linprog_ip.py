...
def _linprog_ip(c, c0, A, b, callback, postsolve_args, maxiter=1000, tol=1e-8,
    ...
    1. ``sksparse.cholmod.cholesky`` (if machine_learning-sparse and SuiteSparse are installed)

    2. ``scipy.sparse.linalg.factorized`` (if machine_learning-umfpack and SuiteSparse are installed)
    ...
    if (cholesky or cholesky is None) and sparse and not has_cholmod:
        if cholesky:
            warn("Sparse cholesky is only available with machine_learning-sparse. "
