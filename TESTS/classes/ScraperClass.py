...
class Scraper:
    ...
    def scrape_page(self, redis_connection: redis.Redis(), timeout: int) -> None:
        ...
        for i in range(0, len(offer_hrefs)):
            ...
            time.sleep(3)
        self.save_results(redis_connection)
        self.driver.close()
...
def scrape(page: int, work: str) -> None:
    ...
    try:
        scraper.scrape_page(redis_connection=redis.Redis(host='127.0.0.1',port=6379), timeout=10)
