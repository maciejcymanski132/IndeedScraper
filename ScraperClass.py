...
import os, pickle, random,  time, zlib
import pandas as pd
import redis
from SwappedFramesClass import *
...
class Scraper:
    def __init__(self, worker_nr: int, search_key: str, location: str):
        ...
        self.driver = webdriver.Firefox(
            seleniumwire_options=self.proxy, log_path=os.path.devnull
        )
    ...
    def save_job_description(self, offer, index: int):
        """
    ...
    def scrape_page(self, timeout: int):
        """
        ...
        for i in range(0, 2):
            ...
            time.sleep(3)
        used_proxies.remove(self.proxy)
        self.save_results()
        self.driver.close()
